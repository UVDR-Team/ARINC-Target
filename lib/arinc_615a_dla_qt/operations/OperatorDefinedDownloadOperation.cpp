...
#include "OperatorDefinedDownloadOperation.hpp"
...
OperatorDefinedDownloadOperation::OperatorDefinedDownloadOperation(
  ...
  configurationV{ std::move( configuration ) }
{
  ...
  qRegisterMetaType< Arinc615a::Information::DownloadFilesInformation >(
    "Arinc615a::Information::DownloadFilesInformation" );
}

void OperatorDefinedDownloadOperation::startOperation(
  ...
  const bool createDownloadDirectory )
{
  try
  {
    ...
    operationV = protocolV->operatorDefinedDownloadOperation(
  ...
  catch ( const boost::exception &e )
  {
    ...
    QMessageBox::critical(
      nullptr,
      tr( "Error during Operator Defined Download Operation" ),
...
void OperatorDefinedDownloadOperation::abortOperation()
{
  try
  {
    if ( operationV )
    {
      operationV->abort( Arinc615a::Host::OperatorDefinedDownloadOperation::AbortReason::Operator );
    }
  }
  catch ( const boost::exception &e )
  {
...
void OperatorDefinedDownloadOperation::transmitFiles( const Arinc615a::Information::DownloadFiles &files )
{
  try
  {
    if ( operationV )
...
void OperatorDefinedDownloadOperation::initialisationDeferred( const std::chrono::seconds waitTime )
{
  emit operationDeferred( waitTime );
}

void OperatorDefinedDownloadOperation::initialisationResponse(
  const Arinc615a::Information::InitializationResponse &response )
{
  emit operationInitialised( response );
}

void OperatorDefinedDownloadOperation::finished( const Arinc615a::StatusCode code, std::string_view description )
{
  try
  {
...
void OperatorDefinedDownloadOperation::status( const Arinc615a::Information::DownloadStatus &status )
{
  emit receivedStatus( status );
}

void OperatorDefinedDownloadOperation::fileRequest(
  ...
  const Arinc649::CheckValue &checkValue )
{
  ...
  fileOperation->completionHandler(
    std::bind_front(
      &OperatorDefinedDownloadOperation::fileCompleted,
      this,
      std::string{ filename },
      filePath,
...
void OperatorDefinedDownloadOperation::downloadingList( const Arinc615a::Information::DownloadFilesInformation &files )
{
  emit receivedDownloadingList( files );
}

void OperatorDefinedDownloadOperation::fileCompleted(
  std::string_view filename,
