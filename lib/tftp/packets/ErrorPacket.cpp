...
ErrorPacket::ErrorPacket( const ArincSupport::ConstRawDataSpan rawPacket ) :
  Packet{ PacketType::Error, rawPacket }
{
  decodeBody( rawPacket );
}

ErrorPacket& ErrorPacket::operator=( const ArincSupport::ConstRawDataSpan rawPacket )
{
...
ArincSupport::RawData ErrorPacket::encode() const
{
  ArincSupport::RawData rawPacket( MinPacketSize + errorMessageV.length() );
  ...
  // error code
  rawSpan = ArincSupport::RawData_setInt( rawSpan, static_cast< uint16_t >( errorCodeV ) );

  // error message
  rawSpan = ArincSupport::RawData_setString( rawSpan, errorMessageV );
  rawSpan.back() = std::byte{ 0 };

  return rawPacket;
}

void ErrorPacket::decodeBody( ArincSupport::ConstRawDataSpan rawPacket )
{
  // check size
  if ( rawPacket.size() < MinPacketSize )
  {
    BOOST_THROW_EXCEPTION( InvalidPacketException{}
      << ArincSupport::AdditionalInfo{ "Invalid packet size of ERROR packet" } );
  }
  ...
  std::tie( rawSpan, errorCodeInt ) = ArincSupport::RawData_getInt< uint16_t >( rawSpan );
  ...
  {
    BOOST_THROW_EXCEPTION( InvalidPacketException{}
      << ArincSupport::AdditionalInfo{ "error message not 0-terminated" } );
  }

  std::tie( rawSpan, errorMessageV ) = ArincSupport::RawData_getString( rawSpan, rawSpan.size() - 1U );
}
