...
ReadWriteRequestPacket::ReadWriteRequestPacket( const PacketType packetType, ArincSupport::ConstRawDataSpan rawPacket ) :
...
void ReadWriteRequestPacket::decodeBody( ArincSupport::ConstRawDataSpan rawPacket )
{
  // check size
  if ( rawPacket.size() <= HeaderSize )
  {
    BOOST_THROW_EXCEPTION( InvalidPacketException()
      << ArincSupport::AdditionalInfo{ "Invalid packet size of RRQ/WRQ packet" } );
  }
  ...
  {
    BOOST_THROW_EXCEPTION( InvalidPacketException()
      << ArincSupport::AdditionalInfo{ "RRQ/WRQ message not 0-terminated" } );
  }

  auto [ _, rawRequestString ]{ ArincSupport::RawData_getString( rawSpan, rawSpan.size() ) };
  ...
  {
    BOOST_THROW_EXCEPTION( InvalidPacketException()
      << ArincSupport::AdditionalInfo{ "No 0-termination for filename found" } );
  }
  ...
  {
    BOOST_THROW_EXCEPTION( InvalidPacketException()
      << ArincSupport::AdditionalInfo{ "No 0-termination for operation found" } );
  }
...
ArincSupport::RawData ReadWriteRequestPacket::encode() const
{
  const auto mode{ decodeMode( modeV ) };
  const auto rawOptions{ Options_rawOptions( optionsV ) };

  ArincSupport::RawData rawPacket( HeaderSize + filenameV.size() + 1U + mode.size() + 1U + rawOptions.size() );
