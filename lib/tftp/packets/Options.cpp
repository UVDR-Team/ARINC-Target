  ...
  for ( auto optionsString{ rawOptions }; !optionsString.empty(); )
  {
    ...
    if ( nameEnd == std::string_view::npos )
    {
      BOOST_THROW_EXCEPTION( Packets::InvalidPacketException()
        << ArincSupport::AdditionalInfo{ "Unexpected end of input data" } );
    }
...
RawOptions Options_rawOptions( const Options &options )
{
  ...
  {
    // option name
    auto rawName{ ArincSupport::RawData_asRawData( name ) };
    ...
    // option value
    auto rawValue{ ArincSupport::RawData_asRawData( option ) };
