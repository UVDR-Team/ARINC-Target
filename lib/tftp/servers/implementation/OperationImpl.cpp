...
void OperationImpl::send( const Packets::Packet &packet )
{
  ...
  try
  {
    ...
    transmitPacket = static_cast< ArincSupport::RawData >( packet );
...
{
  ARINC_LOG_INFO( "TFTP Server operation finished" );
...
{
  ARINC_LOG_ERROR( "RX Error: {}", static_cast< std::string>( optionsAcknowledgementPacket ) );
