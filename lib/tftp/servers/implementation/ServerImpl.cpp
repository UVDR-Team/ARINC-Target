...
{
  ARINC_LOG_INFO( "Start TFTP Server on {}:{}", serverAddressV.address().to_string(), serverAddressV.port() );
  ...
  catch ( const boost::system::system_error &err )
  {
    ...
    BOOST_THROW_EXCEPTION( CommunicationException{}
      << ArincSupport::AdditionalInfo{ err.what() }
...
{
  ARINC_LOG_INFO( "Stop TFTP Server" );
  ...
  std::string errorMessage )
{
  ...
  ARINC_LOG_INFO( "TX: {}", static_cast< std::string>( errorPacket ) );
  ...
  try
  {
    ...
    auto rawPacket{ static_cast< ArincSupport::RawData >( errorPacket ) };
...
void ServerImpl::receiveHandler( const boost::system::error_code &errorCode, const std::size_t bytesTransferred )
{
  ...
  if ( errorCode )
  {
    ARINC_LOG_ERROR( "receive error: {}", errorCode.message() );

    BOOST_THROW_EXCEPTION( CommunicationException{} << ArincSupport::AdditionalInfo{ errorCode.message() } );
  }
  ...
  {
    ARINC_LOG_ERROR( "TFTP exception: {}", e.what() );
  }
...
{
  ARINC_LOG_TRACE( "RX: {}", static_cast< std::string>( readRequestPacket ) );

  // check handler
  if ( !requestHandlerV )
  {
    ARINC_LOG_WARN( "No registered handler - reject" );
...
{
  ARINC_LOG_TRACE( "RX: {}", static_cast< std::string>( writeRequestPacket ) );
...
{
  ARINC_LOG_WARN( "RX Error: {}", static_cast< std::string >( dataPacket ) );
...
{
  ARINC_LOG_WARN( "RX Error: {}", static_cast< std::string >( acknowledgementPacket ) );
...
{
  ARINC_LOG_WARN( "RX Error: {}", static_cast< std::string>( errorPacket ) );
...
{
  ARINC_LOG_WARN( "RX Error: {}", static_cast< std::string >( optionsAcknowledgementPacket ) );
...
{
  ARINC_LOG_WARN( "RX: UNKNOWN: *Error* - IGNORE" );
}
