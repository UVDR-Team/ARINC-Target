...
void WriteOperationImpl::request()
{
  if ( !dataHandlerV )
  {
    BOOST_THROW_EXCEPTION( TftpException{}
      << ArincSupport::AdditionalInfo{ "Parameter invalid" }
  ...
  {
    ARINC_LOG_ERROR( "Exception during request {}", boost::diagnostic_information( e ) );
...
void WriteOperationImpl::sendData()
{
  ...
  ARINC_LOG_TRACE( "Send Data #{}", static_cast< uint16_t >( lastTransmittedBlockNumber ) );
...
{
  ARINC_LOG_ERROR( "RX Error: {}", static_cast< std::string>( dataPacket ) );
...
{
  ARINC_LOG_TRACE( "RX: {}", static_cast< std::string>( acknowledgementPacket ) );
  ...
  {
    ARINC_LOG_ERROR( "Invalid block number received" );
  ...
  if ( acknowledgementPacket.blockNumber() == Packets::BlockNumber{ 0U } )
  {
    ...
    {
      ARINC_LOG_ERROR( "Option Negotiation failed" );
...
{
  ARINC_LOG_TRACE( "RX: {}", static_cast< std::string>( optionsAcknowledgementPacket ) );

  if ( lastReceivedBlockNumber != Packets::BlockNumber{ 0xFFFFU } )
  {
    ARINC_LOG_ERROR( "OACK must occur after WRQ" );
  ...
  {
    ARINC_LOG_ERROR( "Received option list is empty" );
  ...
  {
    ARINC_LOG_ERROR( "Block Size Option isn't expected" );
  ...
  if ( !blockSizeValid )
  {
    ARINC_LOG_ERROR( "Block Size Option decoding failed" );
  ...
  if ( blockSizeValue )
  {
    ...
    {
      ARINC_LOG_ERROR( "Received Block Size Option bigger than negotiated" );
  ...
  {
    ARINC_LOG_ERROR( "Timeout Option not expected" );
  ...
  if ( !timeoutValid )
  {
    ARINC_LOG_ERROR( "Timeout Option decoding failed" );
  ...
  if ( timeoutValue )
  {
    ...
    {
      ARINC_LOG_ERROR( "Timeout option not equal to requested" );
  ...
  {
    ARINC_LOG_ERROR( "Transfer Size Option isn't expected" );
  ...
  {
    ARINC_LOG_ERROR( "Transfer Size Option decoding failed" );
  ...
  {
    ARINC_LOG_ERROR( "Transfer size value is not equal to sent value" );
  ...
  if ( !remoteOptions.empty() )
  {
    ARINC_LOG_ERROR( "Option negotiation failed - unexpected options" );
