 ...
 *
 * @brief Definition of Class Arinc615aCommands::Operations::AdhocUploadOperationCommand.
 **/
...
AdhocUploadOperationCommand::AdhocUploadOperationCommand(
  ...
  optionsDescriptionV{ "ARINC 615A Adhoc Upload Operation Options" }
{
  ...
  (
    ...
    boost::program_options::value( &mediaSourceDirectoriesV )
      ->composing()
      ->required()
      ->value_name( "Medium Source Directory" ),
...
void AdhocUploadOperationCommand::execute( const Commands::Parameters &parameters )
{
  try
  {
    std::cout << "ARINC 615A Adhoc Upload Operation\n";
    ...
    for ( const auto &mediumSourceDirectory : mediaSourceDirectoriesV )
    {
      ...
      mediaPathsV.try_emplace( mediumInformation->mediaSequenceNumber, mediumSourceDirectory );
    }

    if ( mediaPathsV.empty() )
    {
      BOOST_THROW_EXCEPTION( boost::program_options::error{ "No valid media source directories provided" } );
    }
    ...
    if ( !mediaSet )
    {
      BOOST_THROW_EXCEPTION( boost::program_options::error{ "Media Set import failed" } );
    }

    mediaSetV = mediaSet;
    checkValuesV = checkValues;

    // find load headers
    for ( const auto &loadHeader : loadHeadersV )
    {
      auto loadsFound{ mediaSet->recursiveLoads( loadHeader ) };
...
std::filesystem::path AdhocUploadOperationCommand::filePath( const Arinc665::Media::ConstFilePtr &file )
{
  const auto mediumIt{ mediaPathsV.find( file->effectiveMediumNumber() ) };

  if ( mediumIt == mediaPathsV.end() )
  {
    SPDLOG_ERROR( "Medium not found" );
    return {};
  }

  return ( mediumIt->second / file->path().relative_path() ).lexically_normal();
}

void AdhocUploadOperationCommand::loadProgress(
...
void AdhocUploadOperationCommand::initialisationDeferred( const std::chrono::seconds waitTime )
{
  std::cout << std::format( "Operation deferred for: {}\n", waitTime );
}

void AdhocUploadOperationCommand::initialisationResponse( const Arinc615a::Information::InitializationResponse &response )
{
  std::cout << std::format(
...
void AdhocUploadOperationCommand::finished( const Arinc615a::StatusCode code, std::string_view description )
{
  std::cout << std::format(
    "Operation finished:\n"
    "\tFinal Status Code:        {}\n"
...
void AdhocUploadOperationCommand::status( const Arinc615a::Information::UploadStatus &status )
{
  assert( operationV );
...
void AdhocUploadOperationCommand::fileRequest(
  ...
  const Arinc649::CheckValue &checkValue )
{
  ...
  auto realFilename{ filePath( file ) };
  ...
  fileOperation
    ->completionHandler( std::bind_front( &AdhocUploadOperationCommand::fileCompleted, this, fileOperation ) );

  fileOperationsV.emplace_front( std::move( fileOperation ) )->start();
}

void AdhocUploadOperationCommand::fileCompleted(
