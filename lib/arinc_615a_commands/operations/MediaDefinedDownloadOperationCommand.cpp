 ...
 *
 * @brief Definition of Class Arinc615aCommands::Operations::MediaDefinedDownloadOperationCommand.
 **/

#include "MediaDefinedDownloadOperationCommand.hpp"
...
MediaDefinedDownloadOperationCommand::MediaDefinedDownloadOperationCommand(
  ...
  optionsDescriptionV{ "ARINC 615A Media Defined Download Operation Options" }
{
  ...
  (
    "user-defined-data,u",
    boost::program_options::value( &userDefinedDataV )
      ->value_name( "data" ),
    "Additional User-Defined Data transmitted to the Target Data Loader."
  )
  (
...
void MediaDefinedDownloadOperationCommand::execute( const Commands::Parameters &parameters )
{
  try
  {
    std::cout << "ARINC 615A Media Defined Download Operation\n";
    ...
    boost::signals2::scoped_connection abortConnection{ abortSignalV.connect(
      std::bind_front(
        &Arinc615a::Host::MediaDefinedDownloadOperation::abort,
        operationV,
        Arinc615a::Host::MediaDefinedDownloadOperation::AbortReason::Operator ) ) };
    boost::signals2::scoped_connection terminateConnection{ terminateSignalV.connect(
      std::bind_front(
        &Arinc615a::Host::MediaDefinedDownloadOperation::terminate,
        operationV,
        Arinc615a::Host::MediaDefinedDownloadOperation::AbortReason::Operator ) ) };
  ...
  {
    std::cerr << std::format( "Media Defined Download Operation failed: {}\n", boost::diagnostic_information( e ) );
  }
  ...
  {
    std::cerr << "Media Defined Download Operation failed: UNKNOWN EXCEPTION\n";
  }
}

void MediaDefinedDownloadOperationCommand::help() const
{
  std::cout << "Media Defined Download Operation\n";
  std::cout << optionsDescriptionV;
}

void MediaDefinedDownloadOperationCommand::initialisationDeferred( const std::chrono::seconds waitTime )
{
  std::cout << std::format( "Operation deferred for: {}\n", waitTime );
}

void MediaDefinedDownloadOperationCommand::initialisationResponse(
...
void MediaDefinedDownloadOperationCommand::finished( const Arinc615a::StatusCode code, std::string_view description )
{
  std::cout << std::format(
    "Operation finished:\n"
    "\tFinal Status Code:        {}\n"
...
void MediaDefinedDownloadOperationCommand::status( const Arinc615a::Information::DownloadStatus &status )
{
  assert( operationV );
  ...
  if ( ( status.code() == Arinc615a::StatusCode::OperationAccepted ) && !listTransmittedV )
  {
    Arinc615a::Information::DownloadFiles downloadFiles;

    for ( const auto &file : requestedFilesV )
    {
    ...
    Helper::ConstRawDataSpan userDefinedData{
      reinterpret_cast< std::byte const * >( userDefinedDataV.data() ),
      userDefinedDataV.size() };
    operationV->request( std::move( downloadFiles ), { userDefinedData.begin(), userDefinedData.end() } );

    listTransmittedV = true;
  }
}

void MediaDefinedDownloadOperationCommand::fileRequest(
  ...
  const Arinc649::CheckValue &checkValue )
{
  ...
  fileOperation->completionHandler(
    std::bind_front(
      &MediaDefinedDownloadOperationCommand::fileCompleted,
      this,
      std::string{ filename },
      filePath,
...
void MediaDefinedDownloadOperationCommand::fileCompleted(
  std::string_view filename,
