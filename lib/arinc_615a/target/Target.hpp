 ...
 * The Checksum must be empty (@p ArincChecksum::CheckValue::NoCheckValue) or the same as sent.
 *
 * @param[in] partNumber
...
using Arinc615aOptionNegotiationHandler =
  std::function< bool( std::string_view partNumber, const ArincChecksum::CheckValue &checksum ) >;
