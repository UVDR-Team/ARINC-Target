...
BOOST_AUTO_TEST_SUITE( RawData_toRaw2Test )
...
{
  auto data{ RawData_toRaw< uint8_t >( 0x01, boost::endian::order::big ) } ;
...
{
  auto data{ RawData_toRaw< uint8_t >( 0x01, boost::endian::order::little ) } ;
  BOOST_CHECK( data.size() == 1 );
  BOOST_CHECK( std::ranges::equal( data, RawData_asRawData( std::to_array< uint8_t >( { 0x01 } ) ) ) );
}
...
{
  auto data{ RawData_toRaw< uint16_t >( 0x0123, boost::endian::order::big ) } ;
...
{
  auto data{ RawData_toRaw< uint16_t >( 0x0123, boost::endian::order::little ) } ;
  BOOST_CHECK( data.size() == 2 );
...
{
  auto data{ RawData_toRaw< uint32_t >( 0x0123'4567, boost::endian::order::big ) } ;
...
{
  auto data{ RawData_toRaw< uint32_t >( 0x0123'4567, boost::endian::order::little ) } ;
  BOOST_CHECK( data.size() == 4 );
  BOOST_CHECK(
    std::ranges::equal(
      data,
...
{
  auto data{ RawData_toRaw< uint64_t >( 0x0123'4567'89AB'CDEF, boost::endian::order::big ) } ;
...
{
  auto data{ RawData_toRaw< uint64_t >( 0x0123'4567'89AB'CDEF, boost::endian::order::little ) } ;
  BOOST_CHECK( data.size() == 8 );
  BOOST_CHECK(
    std::ranges::equal(
      data,
