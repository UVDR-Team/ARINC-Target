    ...
    socket_type sv[2], boost::system::error_code& ec)
{
  ...
  // VxWorks: socketpair() may not be available.
  // Implement using TCP loopback connect + accept.
  (void)(protocol);
  ...
  if (bind(sv[0], (sockaddr*)&listenAddr, sizeof(listenAddr)) != 0) {
    closesocket(sv[0]);
    ec = boost::asio::error::operation_not_supported;
    return socket_error_retval;
  }
  ...
  if (getsockname(sv[0], (sockaddr*)&listenAddr, &addrLen) != 0) {
    closesocket(sv[0]);
    ec = boost::asio::error::operation_not_supported;
    return socket_error_retval;
  }
  ...
  if (connect(sv[1], (sockaddr*)&listenAddr, addrLen) != 0) {
    closesocket(sv[1]);
    closesocket(sv[0]);
    ec = boost::asio::error::operation_not_supported;
    return socket_error_retval;
  ...
  socket_type accepted = accept(sv[0], (sockaddr*)&clientAddr, &clientAddrLen);
