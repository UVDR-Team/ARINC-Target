...
/* VxWorks-specific poll compatibility layer — maps poll() → select() */
struct pollfd {
  int    fd;
  short  events;
  short  revents;
...
static inline int vxworks_poll(struct pollfd* fds, nfds_t n, int timeout_ms) {
  fd_set    rfds = {}, wfds = {}, efds = {};
  int       max_fd = 0;
  timeval   tv, *tv_ptr = nullptr;
  ...
  for (nfds_t i = 0; i < n; i++) {
    ...
    if (fds[i].events & POLLPRI)   FD_SET(fds[i].fd, &efds);
    if (fds[i].events & POLLERR)   FD_SET(fds[i].fd, &efds);
    if (fds[i].events & POLLHUP)   FD_SET(fds[i].fd, &efds);
  }

  int result = ::select(max_fd + 1, &rfds, &wfds, &efds, tv_ptr);
  if (result <= 0) {
  ...
  for (nfds_t i = 0; i < n; i++) {
    ...
    if (FD_ISSET(fds[i].fd, &efds)) fds[i].revents |= POLLPRI | POLLERR | POLLHUP;
  }
  return 1;
}

#  define poll vxworks_poll
#endif // defined(__VXWORKS__) || defined(__vxworks)

// VxWorks workaround: undefine m_data macro from mbuf.h which collides
// with Boost.PropertyTree member variable names.
