 ...
 *
 * @brief Definition of Class Arinc665Commands::MediaSetManager::ListMediaSetsCommand.
 **/
...
void ListMediaSetsCommand::execute( const Commands::Parameters &parameters )
{
  try
  {
    ...
    if ( const auto mediaSets{ mediaSetManager->mediaSets() }; mediaSets.empty() )
    {
    ...
    else
    {
      for ( const auto &[ partNumber, mediaSet ] : mediaSetManager->mediaSets() )
      {
        ...
        Arinc665::Utils::MediaSetPrinter_print( *mediaSet.first, std::cout, "  ", "  " );
...
void ListMediaSetsCommand::help() const
{
  std::cout
