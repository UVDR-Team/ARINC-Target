...
BatchListFile::BatchListFile( ArincSupport::ConstRawDataSpan rawFile ) :
  ListFile{ rawFile, FileType::BatchList }
{
  decodeBody( rawFile );
}

BatchListFile &BatchListFile::operator=( ArincSupport::ConstRawDataSpan rawFile )
{
...
ArincSupport::ConstRawDataSpan BatchListFile::userDefinedData() const
{
  return userDefinedDataV;
}

void BatchListFile::userDefinedData( ArincSupport::RawData userDefinedData )
{
...
ArincSupport::RawData BatchListFile::encode() const
{
  ...
  ArincSupport::RawData_setInt< uint32_t >(
    ArincSupport::RawDataSpan{ rawFile }.subspan( BatchFilesPointerFieldOffsetV2 ),
    static_cast< uint32_t >( nextFreeOffset / 2U ) );
...
void BatchListFile::decodeBody( ArincSupport::ConstRawDataSpan rawFile )
{
  // Spare Field
  ...
  if ( 0U != spare )
  {
    BOOST_THROW_EXCEPTION(
      InvalidArinc665File() << ArincSupport::AdditionalInfo{ "Spare is not 0" } );
  }
  ...
  // batch list pointer
  auto [ _2, batchListPtr ]{ ArincSupport::RawData_getInt< uint32_t >( rawFile.subspan( BatchFilesPointerFieldOffsetV2 ) ) };
...
ArincSupport::RawData BatchListFile::encodeBatchesInfo() const
{
  ArincSupport::RawData rawBatchesInfo( sizeof( uint16_t ) );

  // Number of batches must not exceed field
  if ( batchesV.size() > std::numeric_limits< uint16_t>::max() )
  {
    BOOST_THROW_EXCEPTION( InvalidArinc665File{} << ArincSupport::AdditionalInfo{ "More batches than allowed" } );
  }

  // number of batches
  ArincSupport::RawData_setInt< uint16_t >( rawBatchesInfo, static_cast< uint16_t >( batchesV.size() ) );
  ...
  for ( auto const &batchInfo : batchesV )
  {
    ...
    ArincSupport::RawData rawBatchInfo( sizeof( uint16_t ) );
    ...
    ArincSupport::RawData_setInt< uint16_t >(
      ArincSupport::RawDataSpan{ rawBatchInfo }.last( sizeof( uint16_t ) ),
...
void BatchListFile::decodeBatchesInfo( ArincSupport::ConstRawDataSpan rawData )
{
  ...
  std::tie( remaining, numberOfBatches ) = ArincSupport::RawData_getInt< uint16_t >( remaining );
  ...
  for ( uint16_t batchIndex = 0; batchIndex < numberOfBatches; ++batchIndex )
  {
    ...
    std::tie( listRemaining, batchPointer ) = ArincSupport::RawData_getInt< uint16_t>( listRemaining );
    ...
    if ( batchIndex != numberOfBatches - 1U )
    {
      if ( batchPointer == 0U )
      {
        BOOST_THROW_EXCEPTION( InvalidArinc665File{} << ArincSupport::AdditionalInfo{ "next batch pointer is 0" } );
      }
    }
    else
    {
      if ( batchPointer != 0U )
      {
        BOOST_THROW_EXCEPTION( InvalidArinc665File{} << ArincSupport::AdditionalInfo{ "next batch pointer is not 0" } );
      }
    }
