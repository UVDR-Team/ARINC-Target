...
FileListFile::FileListFile( ArincSupport::ConstRawDataSpan rawFile ) :
  ListFile{ rawFile, FileType::FileList }
{
  decodeBody( rawFile );
}

FileListFile& FileListFile::operator=( ArincSupport::ConstRawDataSpan rawFile )
{
...
ArincSupport::ConstRawDataSpan FileListFile::userDefinedData() const
{
  return userDefinedDataV;
}

void FileListFile::userDefinedData( ArincSupport::RawData userDefinedData )
{
...
ArincChecksum::CheckValueType FileListFile::checkValueType() const
{
  return checkValueTypeV;
}

void FileListFile::checkValueType( const ArincChecksum::CheckValueType type )
{
...
ArincSupport::RawData FileListFile::encode() const
{
  ...
  // spare field
  ArincSupport::RawData_setInt< uint16_t>( ArincSupport::RawDataSpan{ rawFile }.subspan( SpareFieldOffsetV2 ), 0U );
  ...
  ArincSupport::RawData_setInt< uint32_t >(
    ArincSupport::RawDataSpan{ rawFile }.subspan( MediaSetPartNumberPointerFieldOffsetV2 ),
  ...
  ArincSupport::RawData_setInt< uint32_t >(
    ArincSupport::RawDataSpan{ rawFile }.subspan( MediaSetFilesPointerFieldOffsetV2 ),
    static_cast< uint32_t >( nextFreeOffset / 2U ) );
  ...
  if ( encodeV3Data )
  {
    // Check Value Pointer
    ArincSupport::RawData_setInt< uint32_t >(
      ArincSupport::RawDataSpan{ rawFile }.subspan( FileCheckValuePointerFieldOffsetV3 ),
...
void FileListFile::decodeBody( ArincSupport::ConstRawDataSpan rawFile )
{
  ...
  // Spare Field
  uint16_t spare{};
  std::tie( std::ignore, spare ) = ArincSupport::RawData_getInt< uint16_t >( rawFile.subspan( SpareFieldOffsetV2 ) );

  if ( 0U != spare )
  {
    BOOST_THROW_EXCEPTION( InvalidArinc665File()
      << ArincSupport::AdditionalInfo{ "Spare is not 0" } );
  }

  // media information pointer
  uint32_t mediaInformationPtr{};
  std::tie( std::ignore, mediaInformationPtr ) =
    ArincSupport::RawData_getInt< uint32_t >( rawFile.subspan( MediaSetPartNumberPointerFieldOffsetV2 ) );

  // file list pointer
  uint32_t fileListPtr{};
  std::tie( std::ignore, fileListPtr ) =
    ArincSupport::RawData_getInt< uint32_t >( rawFile.subspan( MediaSetFilesPointerFieldOffsetV2 ) );
  ...
  if ( decodeV3Data )
  {
    std::tie( std::ignore, fileCheckValuePtr ) =
      ArincSupport::RawData_getInt< uint32_t >( rawFile.subspan( FileCheckValuePointerFieldOffsetV3 ) );
  }
  ...
  checkValueTypeV = ArincChecksum::CheckValueType::NotUsed;
  if ( decodeV3Data && ( 0U != fileCheckValuePtr ) )
  {
    ...
    if ( ArincChecksum::CheckValueType::NotUsed != checkValueTypeV )
    {
      ...
      if ( checkValue != calcCheckValue )
      {
        BOOST_THROW_EXCEPTION(
          InvalidArinc665File{} << ArincSupport::AdditionalInfo{ "Check Value Verification failed" } );
      }
    }
  }
...
ArincSupport::RawData FileListFile::encodeFilesInfo( const bool encodeV3Data ) const
{
  ArincSupport::RawData rawFilesInfo( sizeof( uint16_t ) );
  ...
  // number of files
  ArincSupport::RawData_setInt< uint16_t >( rawFilesInfo, static_cast< uint16_t >( filesV.size() ) );
  ...
  for ( auto const &fileInfo : filesV )
  {
    ...
    auto next{ ArincSupport::RawData_setInt< uint16_t>(
      ArincSupport::RawDataSpan{ rawFileInfo }.last( 2 * sizeof( uint16_t ) ),
      static_cast< uint8_t >( fileInfo.memberSequenceNumber ) ) };

    // crc
    ArincSupport::RawData_setInt< uint16_t>( next, fileInfo.crc );
...
void FileListFile::decodeFilesInfo( ArincSupport::ConstRawDataSpan rawData, const bool decodeV3Data )
{
  ...
  for ( unsigned int fileIndex = 0U; fileIndex < numberOfFiles; ++fileIndex )
  {
    ...
    std::tie( listRemaining, memberSequenceNumber ) = ArincSupport::RawData_getInt< uint16_t >( listRemaining );
    if ( ( memberSequenceNumber < 1U ) || ( memberSequenceNumber > 255U ) )
    {
      BOOST_THROW_EXCEPTION( InvalidArinc665File{} << ArincSupport::AdditionalInfo{ "member sequence number out of range" } );
    }
