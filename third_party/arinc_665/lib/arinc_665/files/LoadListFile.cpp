...
LoadListFile::LoadListFile( ArincSupport::ConstRawDataSpan rawFile ) :
  ListFile{ rawFile, FileType::LoadList }
{
  decodeBody( rawFile );
}

LoadListFile& LoadListFile::operator=( ArincSupport::ConstRawDataSpan rawFile )
{
...
ArincSupport::ConstRawDataSpan LoadListFile::userDefinedData() const
{
  return userDefinedDataV;
}

void LoadListFile::userDefinedData( ArincSupport::RawData userDefinedData )
{
...
ArincSupport::RawData LoadListFile::encode() const
{
  ArincSupport::RawData rawFile( FileHeaderSizeV2 );
  ...
  // loads list pointer
  ArincSupport::RawData_setInt< uint32_t >(
    ArincSupport::RawDataSpan{ rawFile }.subspan( LoadFilesPointerFieldOffsetV2 ),
    static_cast< uint32_t >( nextFreeOffset / 2U ) );
...
void LoadListFile::decodeBody( ArincSupport::ConstRawDataSpan rawFile )
{
  // Spare Field
  auto [ _, spare ]{ ArincSupport::RawData_getInt< uint16_t >( rawFile.subspan( SpareFieldOffsetV2 ) ) };
  if ( 0U != spare )
  {
    BOOST_THROW_EXCEPTION( InvalidArinc665File{} << ArincSupport::AdditionalInfo{ "Spare is not 0" } );
  }
  ...
  // Loads list pointer
  auto [ _2, loadListPtr ]{
    ArincSupport::RawData_getInt< uint32_t >( rawFile.subspan( LoadFilesPointerFieldOffsetV2 ) ) };
...
ArincSupport::RawData LoadListFile::encodeLoadsInfo() const
{
  ArincSupport::RawData rawLoadsInfo( sizeof( uint16_t ) );

  // Number of loads must not exceed field
  if ( loadsV.size() > std::numeric_limits< uint16_t>::max() )
  {
    BOOST_THROW_EXCEPTION( InvalidArinc665File()
      << ArincSupport::AdditionalInfo{ "More loads than allowed" } );
  }

  // number of loads
  ArincSupport::RawData_setInt< uint16_t >( rawLoadsInfo, ArincSupport::safeCast< uint16_t >( loadsV.size() ) );
  ...
  for ( auto const &loadInfo : loadsV )
  {
    ...
    ArincSupport::RawData rawLoadInfo( sizeof( uint16_t ) );
    ...
    ArincSupport::RawData_setInt< uint16_t >(
      ArincSupport::RawDataSpan{ rawLoadInfo }.last( sizeof( uint16_t ) ),
...
void LoadListFile::decodeLoadsInfo( ArincSupport::ConstRawDataSpan rawData )
{
  auto remaining{ rawData };

  // number of loads
  uint16_t numberOfLoads{};
  std::tie( remaining, numberOfLoads ) = ArincSupport::RawData_getInt< uint16_t>( remaining );
  ...
  for ( uint16_t loadIndex = 0; loadIndex < numberOfLoads; ++loadIndex )
  {
    ...
    std::tie( listRemaining, loadPointer ) = ArincSupport::RawData_getInt< uint16_t >( listRemaining );
    ...
    if ( loadIndex != numberOfLoads - 1U )
    {
      if ( loadPointer == 0U )
      {
        BOOST_THROW_EXCEPTION( InvalidArinc665File{} << ArincSupport::AdditionalInfo{ "next load pointer is 0" } );
      }
    }
    else
    {
      if ( loadPointer != 0U )
      {
        BOOST_THROW_EXCEPTION( InvalidArinc665File{} << ArincSupport::AdditionalInfo{ "next load pointer is not 0" } );
      }
    }
    ...
    std::tie( listRemaining, fileMemberSequenceNumber ) = ArincSupport::RawData_getInt< uint16_t >( listRemaining );
    ...
    loadsV.emplace_back( LoadInfo{
      ...
      .memberSequenceNumber = MediumNumber{ ArincSupport::safeCast< uint8_t >( fileMemberSequenceNumber ) },
