...
void OperationImpl::initialise()
{
  ...
  {
    ARINC_LOG_ERROR( "Initialisation Error: {}", err.what() );
...
{
  ARINC_LOG_WARN( "Abort requested" );
...
{
  ARINC_LOG_TRACE( "TX: {}", static_cast< std::string>( packet ) );
  ...
  try
  {
    ...
    transmitPacketV = static_cast< ArincSupport::RawData >( packet );
  ...
  {
    ARINC_LOG_ERROR( "TX Error: {}", err.what() );
...
void OperationImpl::receiveFirst()
{
  ...
  {
    ARINC_LOG_ERROR( "RX Error: {}", err.what() );
...
{
  ARINC_LOG_INFO( "TFTP Client Operation finished" );
...
{
  ARINC_LOG_ERROR( "RX Error: {}", static_cast< std::string>( readRequestPacket ) );
...
{
  ARINC_LOG_ERROR( "RX Error: {}", static_cast< std::string>( writeRequestPacket ) );
...
{
  ARINC_LOG_ERROR( "RX Error: {}", static_cast< std::string>( errorPacket ) );
...
{
  ARINC_LOG_ERROR( "RX Error: INVALID Packet" );
  ...
  const std::size_t bytesTransferred )
{
  ...
  if ( errorCode )
  {
    ARINC_LOG_ERROR( "Error when receiving message: {}", errorCode.message() );
  ...
  {
    ARINC_LOG_ERROR( "Received packed from wrong source: {}", receiveEndpointV.address().to_string() );
    ...
    try
    {
      ...
      auto rawPacket{ static_cast< ArincSupport::RawData>( err ) };
    ...
    catch ( const boost::system::system_error &err)
    {
      ...
      ARINC_LOG_ERROR( "Error sending ERR packet: {}", err.what() );
    }
    ...
    {
      ARINC_LOG_ERROR( "Start Receive: {}", err.what() );
  ...
  {
    ARINC_LOG_ERROR( "Connect: {}", err.what() );
...
void OperationImpl::timeoutFirstHandler( const boost::system::error_code &errorCode )
{
  ...
  {
    ARINC_LOG_ERROR( "TFTP Retry counter exceeded" );
  ...
  {
    ARINC_LOG_ERROR( "Re-TX error: {}", err.what() );
...
void OperationImpl::timeoutDallyHandler( const boost::system::error_code &errorCode )
{
  ...
  ARINC_LOG_INFO( "Dally Timeout Completed - Finish" );
