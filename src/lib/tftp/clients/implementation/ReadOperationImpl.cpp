...
{
  ARINC_LOG_TRACE( "RX: {}", static_cast< std::string>( dataPacket ) );

  // Check retransmission of last packet
  if ( dataPacket.blockNumber() == lastReceivedBlockNumber )
  {
    ARINC_LOG_WARN( "Received the last data package again. Re-ACK them." );
  ...
  {
    ARINC_LOG_ERROR( "Wrong Data packet block number" );
  ...
  if ( dataPacket.dataSize() > receiveDataSize )
  {
    ARINC_LOG_ERROR( "Too much data received" );
...
{
  ARINC_LOG_ERROR( "RX Error: {}", static_cast< std::string>( acknowledgementPacket ) );
  ...
  const Packets::OptionsAcknowledgementPacket &optionsAcknowledgementPacket )
{
  ...
  {
    ARINC_LOG_ERROR( "OACK must occur after RRQ" );
