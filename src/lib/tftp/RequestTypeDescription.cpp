...
std::istream& operator>>( std::istream &stream, RequestType &requestType )
{
  ...
  // decode Request Type
  const auto optionalRequestType{ RequestTypeDescription::instance().enumeration( requestTypeStr ) };

  if ( !optionalRequestType )
  {
    BOOST_THROW_EXCEPTION( ArincSupport::InvalidOptionValue{ requestTypeStr } );
  }
