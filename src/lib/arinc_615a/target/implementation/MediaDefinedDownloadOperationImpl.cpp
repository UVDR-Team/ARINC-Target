...
{
  ARINC_LOG_INFO( "Send file {}", filename );
...
void MediaDefinedDownloadOperationImpl::statusFile()
{
  ...
  statusOperation = protocolFileOperation(
    ...
    {}, // Operation deferred is ignored
    ArincSupport::bindFront( &MediaDefinedDownloadOperationImpl::statusFileCompleted, this, statusV ),
    file );
...
void MediaDefinedDownloadOperationImpl::requestFileRequest(
  ...
  Tftp::Arinc615aOptions clientArinc615aOptions )
{
  ARINC_LOG_INFO( "Receive download request file request file" );
  ...
  try
  {
    ...
    ( *requestFileOperation )
      ...
      .completionHandler( ArincSupport::bindFront( &MediaDefinedDownloadOperationImpl::requestFileCompleted, this, memFile ) )
      .dataHandler( memFile )
  ...
  {
    ARINC_LOG_ERROR( "Error receive answer file: {}", boost::diagnostic_information( e ) );
  }
}
  ...
  const ::Tftp::TransferStatus transferStatus )
{
  ...
  ARINC_LOG_INFO( "Receive download request file completed" );
  ...
  try
  {
    ...
    {
      ARINC_LOG_ERROR( "Receive download request file protocol version invalid" );
  ...
  {
    ARINC_LOG_ERROR( "Receive download request file format error: {}", boost::diagnostic_information( e ) );
