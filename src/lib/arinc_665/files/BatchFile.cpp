...
BatchFile::BatchFile( ArincSupport::ConstRawDataSpan rawFile ) :
  Arinc665File{ rawFile, FileType::BatchFile }
{
  decodeBody( rawFile );
}

BatchFile& BatchFile::operator=( ArincSupport::ConstRawDataSpan rawFile )
{
...
ArincSupport::RawData BatchFile::encode() const
{
  ArincSupport::RawData rawFile( BatchFileHeaderSizeV2 );
  ...
  ArincSupport::RawData_setInt< uint32_t >(
    ArincSupport::RawDataSpan{ rawFile }.subspan( BatchPartNumberPointerFieldOffsetV2 ),
...
void BatchFile::decodeBody( ArincSupport::ConstRawDataSpan rawFile )
{
  // Spare field
  ...
  auto [ _1, batchPartNumberPtr]{
    ArincSupport::RawData_getInt< uint32_t >( rawFile.subspan( BatchPartNumberPointerFieldOffsetV2 ) ) };
...
ArincSupport::RawData BatchFile::encodeBatchTargetsInfo() const
{
  ArincSupport::RawData rawBatchTargetsInfo( sizeof( uint16_t ) ); // Number of THW IDs
  ...
  ArincSupport::RawData_setInt< uint16_t>( rawBatchTargetsInfo, ArincSupport::safeCast< uint16_t>( targetsHardwareV.size() ) );
  ...
  for ( auto const &targetHardwareInfo : targetsHardwareV )
  {
    ...
    ArincSupport::RawData rawLoadsInfo{};
    /* iterate over loads */
    ...
    ArincSupport::RawData rawBatchTargetInfo( sizeof( uint16_t ) );
    ...
    ArincSupport::RawData_setInt< uint16_t>(
      ArincSupport::RawDataSpan{ rawBatchTargetInfo }.last( sizeof( uint16_t ) ),
      ArincSupport::safeCast< uint16_t>( targetHardwareInfo.loads.size() ) );
    ...
    ArincSupport::RawData_setInt< uint16_t>(
      ...
      ( thwCounter == targetsHardwareV.size() ) ?
        0U :
        ArincSupport::safeCast< uint16_t>( rawBatchTargetInfo.size() / 2 ) );
...
void BatchFile::decodeBatchTargetsInfo( ArincSupport::ConstRawDataSpan rawData )
{
  ...
  std::tie( remaining, numberOfTargetHardwareIds ) = ArincSupport::RawData_getInt< uint16_t >( remaining );
  ...
  for ( uint16_t thwIdIndex{ 0U }; thwIdIndex < numberOfTargetHardwareIds; ++thwIdIndex )
  {
    ...
    std::tie( listRemaining, thwIdPointer ) = ArincSupport::RawData_getInt< uint16_t>( listRemaining );
    ...
    if ( thwIdIndex != numberOfTargetHardwareIds - 1U )
    {
      ...
      {
        BOOST_THROW_EXCEPTION( InvalidArinc665File()
          << ArincSupport::AdditionalInfo{ "next THW ID pointer is 0" } );
      }
    }
    else
    {
      ...
      {
        BOOST_THROW_EXCEPTION( InvalidArinc665File()
          << ArincSupport::AdditionalInfo{ "next THW ID pointer is not 0" } );
      }
    }
    ...
    std::tie( listRemaining, numberOfLoads ) = ArincSupport::RawData_getInt< uint16_t>( listRemaining );
