...
ArincSupport::RawData ListFile::encodeMediaInformation() const
{
  ...
  auto remaining{ ArincSupport::RawDataSpan{ rawMediaInformation}.subspan( partNumberSize ) };

  // media sequence number
  remaining = ArincSupport::RawData_setInt( remaining, static_cast< uint8_t >( mediaSequenceNumberV ) );

  // number of media set members
  ArincSupport::RawData_setInt< uint8_t>( remaining, static_cast< uint8_t >( numberOfMediaSetMembersV ) );

  return rawMediaInformation;
}

void ListFile::decodeMediaInformation( ArincSupport::ConstRawDataSpan rawData )
{
  ...
  std::tie( remaining, mediaSequenceNumber ) = ArincSupport::RawData_getInt< uint8_t >( remaining );
  mediaSequenceNumberV = mediaSequenceNumber;

  // number of media set members
  uint8_t numberOfMediaSetMembers;
  std::tie( std::ignore, numberOfMediaSetMembers ) = ArincSupport::RawData_getInt< uint8_t >( remaining );
  numberOfMediaSetMembersV = numberOfMediaSetMembers;
}
