  ...
  productIdentifierV{ partNumber.substr( ManufacturerCodeLength + CheckCodeLength, ProductIdentifierLength ) }
{
  ...
  if ( partNumber.size() != Length )
  {
    BOOST_THROW_EXCEPTION( Arinc665Exception()
      << ArincSupport::AdditionalInfo{ "Invalid size of part number string" } );
  }
  ...
  {
    BOOST_THROW_EXCEPTION( Arinc665Exception()
      << ArincSupport::AdditionalInfo{ "calculated and given check code differs" } );
  }
}
catch( const std::out_of_range &e )
{
  BOOST_THROW_EXCEPTION( Arinc665Exception()
    << ArincSupport::AdditionalInfo{ ( e.what() ) } );
}
...
std::string PartNumber::checkCode() const
{
  ...
  return ArincSupport::format( "{:02X}", static_cast< unsigned int >( checkCode ) );
}
...
void PartNumber::checkManufacturerCode( std::string_view manufacturerCode )
{
  ...
  {
    BOOST_THROW_EXCEPTION( Arinc665Exception()
      << ArincSupport::AdditionalInfo{ "length of manufacturer code string invalid" } );
  }
}
...
void PartNumber::checkProductIdentifier( std::string_view productIdentifier )
{
  ...
  {
    BOOST_THROW_EXCEPTION( Arinc665Exception()
      << ArincSupport::AdditionalInfo{ "length of product identifier string invalid" } );
  }
}
...
void PartNumber::checkCheckCode( std::string_view checkCode )
{
  ...
  {
    BOOST_THROW_EXCEPTION( Arinc665Exception()
      << ArincSupport::AdditionalInfo{ "length of check code string invalid" } );
  }
