  ...
  const Packets::ErrorPacket &errorPacket )
{
  ...
  // check for options negotiation error
  if ( ( Packets::PacketType::OptionsAcknowledgement == Packets::Packet::packetType( transmitPacket ) )
    && ( Packets::ErrorCode::TftpOptionRefused == errorPacket.errorCode() ) )
  {
